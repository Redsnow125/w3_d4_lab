...
sayHelloButton.addEventListener('mouseover',buttonColorChanger)
...
// CODE HERE
const buttonRevertColor = () =>{
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color = 'black';
}
sayHelloButton.addEventListener('mouseout',buttonRevertColor)
...
Now lets see if you can send a request param! inside repeatMyParam function below  make get request to 'http://localhost:3000/repeat/{SOMEPARAM}', but with a string instead of {SOMEPARAM}.  
   ...
   const repeatMyParam = (param) => {
       //YOUR CODE HERE
       axios.get(`http://localhost:3000/repeat/Tristam`)
       .then(response =>{
  ...
  const queryTest = () => {
      // YOUR CODE HERE
      axios.get('http://localhost:3000/query-test?dog=yuna&wife=kira')
      .then(response =>{
          console.log(response.data)
   ...
   The query returns as no querys were sent
   2: Send more than 1 query on the URL -- what happened? 
   It shows both the querys and list the querys like an object
   */
    ...
    axios.post('http://localhost:3000/food',body)
         .then((res) =>{
            ...
            res.data.forEach(food => {
                ...
                // document.querySelector('body').appendChild(foodP)
...
let foodtemp = document.querySelector('#food-input')
foodtemp.addEventListener('submit', createFood)